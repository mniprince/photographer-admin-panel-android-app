...
<h3> <a class="btn btn-success float-right" href="top-banner.php">Banner List</a></h3>
</br></br>
<h2>Change Banner</h2>
<div class="row">
...
if(isset($_POST['update'])){
	...
	if($image['size']>1){
		if(file_exists($row['bannerimage']) && !empty($row['bannerimage'])){
			unlink($row['bannerimage']);
		}
		move_uploaded_file($tmp_name, "bannerphoto/".$file_name);
		$update_sql="UPDATE bannertbl SET bannerimage='bannerphoto/$file_name' WHERE id='$id' ";
	}
...
<img id="profileDisplay" src="<?php echo $row['bannerimage']; ?>" onclick="triggerClick()" style="height:100px; width:100px; border:1px solid black"/>
